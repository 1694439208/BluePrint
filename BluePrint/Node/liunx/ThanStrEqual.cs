...
{
    [NodeBaseInfo("等于 =", "字符逻辑运算")]
    public class ThanStrEqual : NodeBase
    {
        protected override void InitializeComponent()
        {
            base.InitializeComponent();
        }
        public ThanStrEqual(BParent _bParent):base(_bParent) {
            Title = "等于 =";
        ...
        public override string CodeTemplate(List<string> Execute, List<string> PrevNodes, List<ParameterAST> arguments, List<ParameterAST> result)
        {
            return $@"
{PrevNodes.join("\r\n")}
{result[0].ID.GetID(false)}=test {arguments[0].GetData(a => a.Value)} == {arguments[1].GetData(a => a.Value)}";
        }
    }
}
